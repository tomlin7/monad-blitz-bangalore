...
export function MoveOutPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-8">
      ...
      {!settlement ? (
        <>
          <div className="neo-container bg-neo-accent-pink text-white text-center">
            <h3 className="font-pixel text-sm mb-4">UPLOAD CURRENT CONDITION</h3>
            <div 
              onClick={() => setImagesUploaded(true)}
              className="neo-inner-card bg-white text-black border-dashed border-4 flex flex-col items-center justify-center h-48 cursor-pointer hover:bg-gray-50"
            >
              {!imagesUploaded ? (
                <>
                ...
                <>
                  <span className="text-4xl mb-2">✅</span>
                  <p className="font-pixel text-[10px] text-neo-accent-green">EVIDENCE READY</p>
                </>
              )}
            </div>
          </div>
